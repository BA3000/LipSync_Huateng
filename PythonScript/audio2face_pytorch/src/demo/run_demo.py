...
async def ws_sent_bs_json_data(websocket, path):
    fps = 30
    ...
    for i, (need_play, wav_path) in enumerate(wav_path_list):
        ...
        with open(json_bs_file, "r", encoding="utf-8") as f:
            ...
            for line in f.readlines():
                ...
                tmp2 = tmp.split("#")[2]
                print("=== " + tmp2)
                await websocket.send(tmp2.strip().encode())
    ...
    for i, (need_play, wav_path) in enumerate(wav_path_list):
        ...
        with open(json_bs_file, "r", encoding="utf-8") as f:
            ...
            for line in f.readlines():
                ...
                tmp = line.strip()
                tmp2 = tmp.split("#")[2]
                # _client.send(tmp2.encode())
                # _client.send(line.strip().encode())
                msg = tmp2.encode()
                msgLen = len(tmp2.encode())
                finalMsg = msgLen.to_bytes(2, byteorder="little") + msg
                # print(finalMsg)
                _client.send(finalMsg)
                sleep_time = max(speed_play * f_num - (time.time() - f_btime), 0.)
                if sleep_time <= 0:
                    print("blendshape delay, {}".format(f_num))
                time.sleep(sleep_time)
